...
import { PenTool, Loader2, Search, ArrowRight, Filter } from 'lucide-react';
...
const ApproveRentals = () => {
  ...
  const { data: rentals = [], isLoading } = useQuery({
    queryKey: ['approveRentals', statusFilter],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Dashboard Persetujuan</h1>
        <p className="text-slate-500 text-sm mt-1">Kelola keputusan persetujuan untuk seluruh pengajuan sewa aset.</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <option value="PENDING_APPROVAL">Menunggu Persetujuan (Aktif)</option>
            <option value="APPROVED">Telah Disetujui</option>
            <option value="REVISION">Diminta Revisi</option>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
          {filteredRentals.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-slate-500 text-sm mt-1 mb-6">Tidak ada data yang cocok dengan kriteria Anda.</p>
            </div>
          ) : (
                  ...
                  {filteredRentals.map((rental) => (
                    <tr key={rental.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <div className="text-[10px] text-slate-400 mt-1">{new Date(rental.updatedAt).toLocaleDateString()}</div>
                      </td>
