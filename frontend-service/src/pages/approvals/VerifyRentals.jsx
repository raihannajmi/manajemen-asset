...
import { ClipboardCheck, Loader2, Search, ArrowRight } from 'lucide-react';
import { Link } from 'react-router-dom';
import api from '../../lib/axios';

const VerifyRentals = () => {
  const { data: rentals = [], isLoading } = useQuery({
    queryKey: ['verifyRentals'],
    queryFn: async () => {
      // Fetch only SUBMITTED rentals for Admin
      const res = await api.get('/rentals?status=SUBMITTED');
      return res.data;
    }
  });
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Verifikasi Pengajuan Masuk</h1>
        <p className="text-slate-500 text-sm mt-1">Periksa kelengkapan dokumen dan teruskan ke Pimpinan.</p>
      </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
          {rentals.length === 0 ? (
            <div className="text-center py-20">
              ...
              <h3 className="text-lg font-medium text-slate-900">Tidak ada antrean verifikasi</h3>
              <p className="text-slate-500 text-sm mt-1 mb-6">Semua pengajuan masuk sudah diproses.</p>
            </div>
          ) : (
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Tanggal Masuk</th>
                ...
                {rentals.map((rental) => (
                  <tr key={rental.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                      {new Date(rental.submittedAt || rental.createdAt).toLocaleString('id-ID')}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <Link to={`/rentals/${rental.id}`} className="inline-flex items-center text-blue-600 hover:text-blue-900 font-semibold bg-blue-50 px-4 py-2 rounded-lg hover:bg-blue-100 transition-colors">
                        Review Dokumen <ArrowRight size={16} className="ml-2" />
                      </Link>
                    </td>
                  </tr>
                ))}
